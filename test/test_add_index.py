...
from db import add_index, get_execution_time, drop_indexes, get_estimated_execution_time
...
class TestAdd_index(TestCase):
    def test_add_index(self):
        ...
        for query in queries:
            ...
            execution_time_before = get_estimated_execution_time(connector, query['query'])
            for index in indexes_to_add:
                add_index(connector, index, table_name)
            execution_time_after = get_estimated_execution_time(connector, query['query'])
            drop_indexes(connector, table_name)
            print(execution_time_before, execution_time_after)
